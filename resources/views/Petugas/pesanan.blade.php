    ...
    @section('content')
            ...
            <form method="GET" action="{{ route('petugas.pesanan') }}" class="mb-6">
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="w-full sm:w-48">
                        <select name="status" class="w-full p-3 border border-gray-300 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary focus:ring-opacity-20 smooth-transition">
                    ...
                    <div class="flex gap-2">
                        <button type="submit" class="bg-primary hover:bg-primary-dark text-white px-4 py-3 rounded-lg smooth-transition">
                            Filter
                        </button>
                        <a href="{{ route('petugas.pesanan') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-3 rounded-lg smooth-transition">
                            Reset
                        </a>
                    </div>
                </div>
                            ...
                            <tr id="order-row-{{ $order->id }}">
                                ...
                                <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <!-- Button View Detail -->
                                        <a href="{{ route('petugas.pesanan.detail', $order->id) }}" 
                                        ...
                                        @if($order->status == 'pending')
                                            <!-- Button Terima -->
                                            <button onclick="showConfirmationModal({{ $order->id }}, 'dikonfirmasi', 'Terima')" 
                                            ...
                                            <!-- Button Tolak -->
                                            <button onclick="showConfirmationModal({{ $order->id }}, 'dibatalkan', 'Tolak')" 
                                        ...
                                        @elseif($order->status == 'dikonfirmasi')
                                            <!-- Button Mulai Proses -->
                                            <button onclick="showConfirmationModal({{ $order->id }}, 'diproses', 'Mulai Proses')" 
                                        ...
                                        @elseif($order->status == 'diproses')
                                            <!-- Button Selesai -->
                                            <button onclick="showConfirmationModal({{ $order->id }}, 'selesai', 'Selesaikan')" 
                                                    class="text-green-600 hover:text-green-800"
                                                    title="Selesai">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <div class="items-center px-4 py-3">
                        <button id="confirmButton" class="px-4 py-2 bg-blue-500 text-white text-base font-medium rounded-md w-24 mr-2 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300">
                            Ya
                        </button>
                        <button id="cancelButton" class="px-4 py-2 bg-gray-300 text-gray-800 text-base font-medium rounded-md w-24 hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300">
                            Batal
                        </button>
                    </div>
                </div>
        ...
        <script>
            ...
            function showConfirmationModal(orderId, newStatus, actionName) {
                ...
                confirmButton.className = `px-4 py-2 text-white text-base font-medium rounded-md w-24 mr-2 focus:outline-none focus:ring-2 ${buttonColors[newStatus]}`;
            ...
            function updateOrderStatus() {
                ...
                .catch(error => {
                    ...
                    showErrorMessage('Terjadi kesalahan saat mengubah status. Silakan coba lagi.');
                });
            }
            ...
            function showSuccessMessage(message) {
                ...
                alertDiv.className = 'fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 transform transition-all duration-300 translate-x-full';
                alertDiv.innerHTML = `
            ...
            function showErrorMessage(message) {
                const alertDiv = document.createElement('div');
                alertDiv.className = 'fixed top-4 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 transform transition-all duration-300 translate-x-full';
                alertDiv.innerHTML = `
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
