...
@section('content')
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        @if(session('success'))
            <div id="success-alert" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4" role="alert">
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                    ...
                    <div class="grid md:grid-cols-2 gap-4 mb-6" id="packageSelection">
                        @forelse($paketJasas as $paket)
                            <div class="package-card border-2 border-gray-200 rounded-xl p-4 cursor-pointer transition-all duration-300 card-hover"
                                 ...
                                 data-package-price="{{ $paket->harga }}"
                                ...
                                <div class="flex items-center mb-3">
                                    ...
                                    ">
                                        ...
                                        @elseif(stripos($paket->nama_paket, 'gudang') !== false)
                                            ...
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M12 2L2 7v10c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7L12 2zm6 15H6v-8l6-3 6 3v8z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'sapu') !== false || stripos($paket->nama_paket, 'pel') !== false)
                                            {{-- Sweeping and mopping icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M19.36 2.72L20.78 4.14l-1.42 1.42c-.45-.51-1.12-.75-1.74-.49L6.5 9.5c-.69.26-1.04 1.03-.78 1.72L7.14 14l-4.92 4.92c-.39.39-.39 1.02 0 1.41.39.39 1.02.39 1.41 0L8.56 15.3c.69.26 1.46-.09 1.72-.78l4.43-11.12c.26-.62.02-1.29-.49-1.74l1.42-1.42zM9.5 10.5c-.28 0-.5-.22-.5-.5s.22-.5.5-.5.5.22.5.5-.22.5-.5.5z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'kaca') !== false || stripos($paket->nama_paket, 'jendela') !== false)
                                            {{-- Window cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM11 19H4V5h7v14zm9 0h-7V5h7v14zm-6-12h4v2h-4V7zm0 4h4v2h-4v-2z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'kamar mandi') !== false || stripos($paket->nama_paket, 'toilet') !== false)
                                            {{-- Bathroom cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M6,2V8H4V7H2V8C2,9.11 2.89,10 4,10H6C7.11,10 8,9.11 8,8V7H6V2M9,12A3,3 0 0,1 12,9A3,3 0 0,1 15,9A3,3 0 0,1 18,12V13H22V20C22,21.11 21.11,22 20,22H4C2.89,22 2,21.11 2,20V13H6V12A3,3 0 0,1 9,12M12,11A1,1 0 0,0 11,12V13H13V12A1,1 0 0,0 12,11Z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'dapur') !== false)
                                            {{-- Kitchen cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M18,3H6A3,3 0 0,0 3,6V18A3,3 0 0,0 6,21H18A3,3 0 0,0 21,18V6A3,3 0 0,0 18,3M18,19H6A1,1 0 0,1 5,18V8H19V18A1,1 0 0,1 18,19M19,6H5V6A1,1 0 0,1 6,5H18A1,1 0 0,1 19,6M12,10A2,2 0 0,0 10,12A2,2 0 0,0 12,14A2,2 0 0,0 14,12A2,2 0 0,0 12,10Z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'taman') !== false || stripos($paket->nama_paket, 'halaman') !== false)
                                            {{-- Garden/yard cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M15.4,9.02L16.78,10.4L12,15.18L7.22,10.4L8.6,9.02L12,12.42L15.4,9.02Z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'furniture') !== false || stripos($paket->nama_paket, 'mebel') !== false)
                                            {{-- Furniture cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M3,9V15H4V19H6V15H18V19H20V15H21V9H3M5,11H8V13H5V11M10,11H14V13H10V11M16,11H19V13H16V11Z"/>
                                            </svg>
                                        @elseif(stripos($paket->nama_paket, 'general') !== false || stripos($paket->nama_paket, 'umum') !== false)
                                            {{-- General cleaning icon --}}
                                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M19.36,2.72L20.78,4.14L12,12.92L3.22,4.14L4.64,2.72L12,10.08L19.36,2.72M6,16V18H18V16C18,15.65 17.9,15.31 17.71,15.03L16.83,13.86C16.28,13.12 15.39,12.68 14.45,12.68H9.55C8.61,12.68 7.72,13.12 7.17,13.86L6.29,15.03C6.1,15.31 6,15.65 6,16Z"/>
                                            </svg>
                                        @else
                                    ...
                                    <div>
                                        ...
                                        <p class="text-primary font-bold">Rp {{ number_format($paket->harga, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                                    ...
                                    @else
                                        ...
                                        @else
                                            <li class="flex items-center text-gray-500">
                                                <svg class="w-3 h-3 text-gray-400 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6" id="customRequestForm" style="display: {{ old('is_custom_request') == 1 ? 'block' : 'none' }};">
                    <h3 class="text-xl font-semibold text-gray-800 mb-4 flex items-center">
                        <svg class="w-5 h-5 text-primary mr-2" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <textarea id="customRequestText" name="custom_request" rows="4" class="w-full border border-gray-300 rounded-lg p-3 focus:border-primary focus:ring-2 focus:ring-primary focus:ring-opacity-20 transition duration-200" placeholder="Jelaskan detail permintaan kebersihan Anda...">{{ old('custom_request') }}</textarea>
                    <div id="customRequestImage" class="mt-4">
                        ...
                        <input type="file" id="imageInput" name="image" accept="image/*" class="w-full border border-gray-300 rounded-lg p-3 focus:border-primary focus:ring-2 focus:ring-primary focus:ring-opacity-20 transition duration-200">
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-8">
                    ...
                    <form id="orderForm" action="{{ route('pesan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <input type="hidden" name="paket_id" id="selectedPackageId" value="{{ old('paket_id') }}">
                        <input type="hidden" name="is_custom_request" id="isCustomRequest" value="{{ old('is_custom_request', 0) }}">    
                        ...
                        <div id="customRequestFormInputs" style="display: {{ old('is_custom_request') == 1 ? 'block' : 'none' }};">
                            <input type="hidden" name="custom_request" id="hiddenCustomRequestText">
                            <input type="hidden" name="custom_image_uploaded" id="hiddenCustomImageUploaded"> {{-- To track if an image was selected --}}
                        </div>

                        <button type="button" id="submitOrderBtn" class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary-dark transform hover:scale-105 transition duration-200 shadow-lg hover:shadow-xl">
                            Kirim Pesanan
                        </button>
                    </form>
    ...
    <div id="confirmationModal" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg p-8 max-w-sm mx-auto shadow-xl">
            ...
            <div class="flex justify-end space-x-4">
                <button type="button" id="cancelOrderBtn" class="px-6 py-2 rounded-lg text-gray-600 border border-gray-300 hover:bg-gray-100 transition duration-200">Batal</button>
                <button type="button" id="confirmOrderBtn" class="px-6 py-2 rounded-lg bg-primary text-white hover:bg-primary-dark transition duration-200">Ya, Kirim</button>
            </div>
        </div>
    </div>
    ...
    function updateEstimatedPrice() {
        const packagePrice = selectedPackageId.value ? parseInt(document.querySelector(`[data-package-id="${selectedPackageId.value}"]`).dataset.packagePrice) : 0;
    ...
    removePackageBtn.addEventListener('click', function() {
        ...
        updateEstimatedPrice(); // Update estimated price after removing package (akan jadi 0 jika tidak ada custom request)
    });
