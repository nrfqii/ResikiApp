...
<div class="flex min-h-screen" x-data="{ 
    ...
    <div class="sidebar-container fixed inset-y-0 left-0 z-50 gradient-bg text-white flex flex-col shadow-2xl smooth-transition"
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 rounded-xl overflow-hidden shadow-lg ring-2 ring-white/20">
                        <img src="{{ asset('LogoResiki.jpeg') }}" alt="Logo ResikiApp" class="w-full h-full object-cover" />
                    </div>
        ...
        <div class="p-6 border-b border-white/20">
            <div class="flex items-center space-x-3">
                <div class="w-12 h-12 glass-effect rounded-full flex items-center justify-center user-avatar smooth-transition">
        ...
        <nav class="flex-1 p-4 space-y-2 custom-scrollbar overflow-y-auto">
            ...
            @if($role === 'konsumen')
                <div class="space-y-2">
                    <div class="px-3 py-2 text-xs font-semibold text-white/60 uppercase tracking-wider nav-text smooth-transition">
                        Menu Utama
                    </div>
                    <a href="/dashboard/konsumen" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('dashboard/konsumen') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                    ...
                    <a href="/pesan" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('pesan*') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(isset($notificationCount) && $notificationCount > 0)
                            <div class="ml-auto nav-text smooth-transition">
                                <span class="bg-red-500 text-white text-xs px-2 py-1 rounded-full pulse-animation">{{ $notificationCount }}</span>
                            </div>
                        @endif
                        <div class="tooltip">Pesan Jasa</div>
                    </a>
                    <a href="/ulasan" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('ulasan*') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @elseif($role === 'petugas')
                <div class="space-y-2">
                    ...
                    <a href="/dashboard/petugas" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('dashboard/petugas') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="/petugas/pesanan" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('petugas/pesanan*') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(isset($pendingOrdersCount) && $pendingOrdersCount > 0)
                            <div class="ml-auto nav-text smooth-transition">
                                <span class="bg-yellow-500 text-white text-xs px-2 py-1 rounded-full pulse-animation">{{ $pendingOrdersCount }}</span>
                            </div>
                        @endif
                        <div class="tooltip">Pesanan Masuk</div>
                    </a>
                    <a href="/petugas/riwayat" class="nav-item flex items-center px-4 py-3 rounded-xl smooth-transition relative {{ request()->is('petugas/riwayat*') ? 'bg-white/20 shadow-lg' : 'hover:bg-white/10' }}">
                        <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 border-t border-white/20">
            <form action="/logout" method="POST" class="w-full">
                @csrf
                <button type="submit" class="w-full nav-item flex items-center px-4 py-3 rounded-xl hover:bg-red-500/20 text-red-100 smooth-transition group relative">
                    <svg class="w-5 h-5 mr-3 group-hover:rotate-12 smooth-transition flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex justify-between items-center h-16">
                    <div class="flex items-center space-x-4">
                        ...
                        <button @click="sidebarCollapsed = !sidebarCollapsed"
                                ...
                                x-show="!isMobile">
                            <svg class="w-6 h-6 smooth-transition" :class="{ 'rotate-180': sidebarCollapsed }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center space-x-4">
                        <button class="relative p-2 text-gray-400 hover:text-[#0ABAB5] smooth-transition">
                            ...
                            @if(isset($totalNotifications) && $totalNotifications > 0)
                                <span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-xs rounded-full flex items-center justify-center pulse-animation">{{ $totalNotifications }}</span>
                            @endif
                        </button>

                        <div class="hidden sm:flex items-center space-x-2 text-sm">
                            <span class="text-gray-600">Halo,</span>
                            <span class="font-semibold text-[#0ABAB5]">{{ auth()->user()->name ?? 'Guest' }}</span>
                        </div>
                    </div>
                </div>
            </div>
